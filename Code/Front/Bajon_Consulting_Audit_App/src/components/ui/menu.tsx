...
const MenuIcon = forwardRef<MenuIconHandle, MenuIconProps>(
  ({ onMouseEnter, onMouseLeave, className, size = 28, open, ...props }, ref) => {
    const controls = useAnimation();
    const isControlledRef = useRef(false);

    // keep animation in sync with external `open` prop when provided
    // if `open` is true we animate to the 'animate' variant, otherwise 'normal'
    // use a micro effect so external state always wins
    React.useEffect(() => {
